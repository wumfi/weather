...
$debugfile='/var/www/html/weather/debug.txt';
...
if ($last != $condition) {
	...
	echo $condition;
	$debugtext="Result sent: {$condition}".PHP_EOL;
	if($debugfileon==1) {
		file_put_contents($debugfile,$debugtext,FILE_APPEND);
	}
	...
	echo $last;
	$debugtext="Result sent: {$last}".PHP_EOL;
	if($debugfileon==1) {
		file_put_contents($debugfile,$debugtext,FILE_APPEND);
	}
}
